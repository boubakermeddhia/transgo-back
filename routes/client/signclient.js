    ...
    try {
        ...
        if (!existinguser || params.numerotel == existinguser.numerotel) {
            if (params.password.length != 0) {
                ...
                var updateuser = await Client.findByIdAndUpdate(id, {
...
router.route('/payment').post(Auth, async (req, res) => {
    ...
    try {
        ...
        if (admin.secure == "2af264b99ff1d93e9477482ed9037db8") {
            const existinguser = await Client.findById(use)
            if (existinguser) {
                const updateuser = await Client.findByIdAndUpdate(existinguser._id, {
